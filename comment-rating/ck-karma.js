...
function ckratingKarma(id, action, path, imgIndex) {
    ...
    votetimeout = window.setTimeout(function(){votetimeout = null;}, 2000);
            ...
            if (response[1]) {
                ...
                if (response[4] == 'down') {
                    if (document.getElementById("up-" + response[1]) != null) {
                        jQuery("#up-" + response[1]).addClass("notvoted");
                        jQuery("#up-" + response[1]).removeClass("voted");
                        jQuery("#up-" + response[1]).text('Support Location!');
                    }
                }
                else {
                ...
                if (response[4] == 'up') {
                    if (document.getElementById("up-" + response[1]) != null) {
                        jQuery("#up-" + response[1]).addClass("voted");
                        jQuery("#up-" + response[1]).removeClass("notvoted");
                        jQuery("#up-" + response[1]).text('Supported!');
                    }
                }
                else {
                    if (document.getElementById("up-" + response[1]) != null) {
                ...
                if (document.getElementById("karma-" + response[1] + "-up") != null) {
                    if (karmanumber == "1")
                    document.getElementById("karma-" + response[1] + "-up").innerHTML = karmanumber + " Supporter";
                    else
                    document.getElementById("karma-" + response[1] + "-up").innerHTML = karmanumber + " Supporters";
                }
                ...
                var marker = getMarker(response[1]);
                
                if (marker) {
                    marker.html = jQuery("<div>").append(jQuery(".infowindow-container").clone()).remove().html();
                    infowindow.setContent(marker.html);
                    
                    registerVoteListener(marker, $(marker.html).find(".vote-bttn"));
                }
